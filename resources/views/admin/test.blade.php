...
$list = DB::select('select * from mindkraft18_payment_info');
foreach ($list as $item) {
  $new = implode(':', array_unique(explode(':', $item->payed_for)));
  DB::statement('update mindkraft18_payment_info set payed_for=\''.$new.'\' where id=\''.$item->id.'\'');
}
