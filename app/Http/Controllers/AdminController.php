...
class AdminController extends Controller
{
  ...
  public function approveUser(Request $request)
  {
    ...
    try {
      // If user is not approved -> add user to approved list and payment list
      if (!checkUserStatus($user->id)) {
        DB::insert('insert into mindkraft18_approved_enduser values(?)', [$user->id]);
        DB::insert('insert into mindkraft18_payment_info values(?, ?)', [$user->id, 'main:'.implode(':', $workshop_array)]);
        ...
        $new = implode(':', array_unique(explode(':', $payed_for . implode(':', $workshop_array)));
